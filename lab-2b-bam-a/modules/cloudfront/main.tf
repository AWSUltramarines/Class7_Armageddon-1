...
resource "aws_cloudfront_distribution" "cf_distro" {
  ...
  # C) Ordered Behavior: Safe caching for /health (origin-controlled)
  ordered_cache_behavior {
    path_pattern           = "/health"
    target_origin_id       = "${var.name_prefix}-alb-origin"
    allowed_methods        = ["GET", "HEAD"]
    ...
    cache_policy_id          = aws_cloudfront_cache_policy.origin_safe.id
    origin_request_policy_id = aws_cloudfront_origin_request_policy.static_request.id
  }

  # D) Lab 2B-Honors: /api/public-feed = origin-driven caching
  #    Uses AWS managed UseOriginCacheControlHeaders policy.
  #    CloudFront only caches if origin explicitly sends Cache-Control.
  ordered_cache_behavior {
  ...
  # E) Ordered Behavior: /api/* = safe default (no caching)
  #    Everything else under /api is not cached until proven safe.
  ordered_cache_behavior {
    path_pattern           = "/api/*"
    target_origin_id       = "${var.name_prefix}-alb-origin"
    allowed_methods        = ["DELETE", "GET", "HEAD", "OPTIONS", "PATCH", "POST", "PUT"]
...
# 2. Cache Policy: Origin-Controlled "Safe Caching"
#    default_ttl=0 means CloudFront won't cache UNLESS the origin
#    explicitly sends a Cache-Control header. The origin is the authority.
resource "aws_cloudfront_cache_policy" "origin_safe" {
  name        = "${var.name_prefix}-origin-safe-cache"
  comment     = "Safe caching: respects origin Cache-Control, never caches by default"
  default_ttl = 0     # Don't cache unless origin says to
  max_ttl     = 86400 # Allow up to 24h if origin requests it
  min_ttl     = 0     # Allow origin to say no-store/no-cache
...
# 3. Cache Policy for API (Disabled)
