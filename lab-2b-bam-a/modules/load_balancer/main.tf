...
resource "aws_lb_listener" "redirect" {
  ...
  # Default: Block direct access to Port 80
  default_action {
    type = "fixed-response"
    fixed_response {
      content_type = "text/plain"
      message_body = "Access Denied: Use the CloudFront domain."
      status_code  = "403"
    }
  }
}

# Add a rule to Port 80 that also allows the CloudFront secret
resource "aws_lb_listener_rule" "require_origin_header_http" {
