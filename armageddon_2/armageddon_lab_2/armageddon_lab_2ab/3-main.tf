...
# Commented out — ALB ingress now restricted to CloudFront prefix list only.
# See lab2_cloudfront_origin_cloaking.tf for the replacement rule.
...
resource "aws_lb_listener" "https_listener01" {
  ...
  # Default action changed to 403 — CloudFront adds the X-Jasongeddon-Growl header,
  # and listener rules in lab2_cloudfront_origin_cloaking.tf forward matching requests.
  # Any request without the header gets blocked here.
  default_action {
    type = "fixed-response"
    fixed_response {
