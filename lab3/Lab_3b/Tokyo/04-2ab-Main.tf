...
resource "aws_s3_bucket" "helga_alb_logs_bucketlab3" {
...
resource "aws_s3_bucket_policy" "helga_alb_logs_policylab3" {
  count  = var.enable_alb_access_logs ? 1 : 0
  bucket = aws_s3_bucket.helga_alb_logs_bucketlab3[0].id
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
        ...
        Action   = "s3:PutObject"
        Resource = "${aws_s3_bucket.helga_alb_logs_bucketlab3[0].arn}/${var.alb_access_logs_prefix}/AWSLogs/${data.aws_caller_identity.current.account_id}/*"
      },
      {
      ...
      },
      {
        ...
        Resource = aws_s3_bucket.helga_alb_logs_bucketlab3[0].arn
      }
    ]
  })
} 
...
resource "aws_lb" "helga_alblab3" {
  name               = "${var.project_name}-alblab3"
  internal           = false
  load_balancer_type = "application"
  security_groups    = [aws_security_group.helga_alb_sglab3.id]
  
subnets            = [aws_subnet.helga_public_subnets[0].id, aws_subnet.helga_public_subnets[1].id]

  # ADD THIS BLOCK ⬇️
  access_logs {
    bucket  = var.enable_alb_access_logs ? aws_s3_bucket.helga_alb_logs_bucketlab3[0].bucket : ""
...
resource "aws_security_group" "helga_alb_sglab3" {
  name        = "${var.project_name}-alb-sglab3"
...
#This block commented out becuase it is not needed since we are using cloudfront with the ALB
 /*  # HTTP from CloudFront only (for redirect)
  ...
  tags = { Name = "${var.project_name}-alb-sglab3" }
}
...
resource "aws_lb_listener" "helga_https_listenerlab3" {
  load_balancer_arn = aws_lb.helga_alblab3.arn
  port              = 443
  protocol          = "HTTPS"
  ssl_policy        = "ELBSecurityPolicy-TLS13-1-2-2021-06"
  certificate_arn   = aws_acm_certificate_validation.helga_alb_cert_validationlab3.certificate_arn
...
/* resource "aws_lb" "helga_alblab3" {
  ...
  tags               = { Name = "${var.project_name}-alblab3" }
} 

resource "aws_lb_target_group" "helga_tglab3" {
  name     = "${var.project_name}-tglab3"
  port     = 80
  protocol = "HTTP"
  vpc_id   = aws_vpc.helga_vpclab3.id
...
resource "aws_lb_target_group_attachment" "helga_tg_attachlab3" {
  target_group_arn = aws_lb_target_group.helga_tglab3.arn
  target_id        = aws_instance.helga_ec2lab3.id # Ensure this matches your Private EC2 resource name
  port             = 80
}


# Listener Rule: If secret header matches → forward to target group
resource "aws_lb_listener_rule" "helga_origin_verify_rulelab3" {
  listener_arn = aws_lb_listener.helga_https_listenerlab3.arn
  priority     = 1
...
resource "aws_lb_listener" "helga_http_listenerlab3" {
  load_balancer_arn = aws_lb.helga_alblab3.arn
  port              = 80
  protocol          = "HTTP"
...
############################################
# This is SEPARATE from the CloudFront cert (ap-northeast-1)
# ALB needs a valid cert for CloudFront → ALB HTTPS connection

resource "aws_acm_certificate" "helga_alb_certlab3" {
  domain_name               = var.domain_name
  subject_alternative_names = [local.helga_app_fqdn]
  validation_method         = "DNS"

  tags = { Name = "${var.project_name}-alb-certlab3" }
...
resource "aws_route53_record" "helga_alb_cert_validation" {
  for_each = {
    for dvo in aws_acm_certificate.helga_alb_certlab3.domain_validation_options : dvo.domain_name => {
...
resource "aws_acm_certificate_validation" "helga_alb_cert_validationlab3" {
  certificate_arn         = aws_acm_certificate.helga_alb_certlab3.arn
