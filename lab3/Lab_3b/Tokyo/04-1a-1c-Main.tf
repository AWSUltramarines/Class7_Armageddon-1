...
resource "aws_eip" "helga_nat_eiplab3" {
  domain = "vpc"

  tags = {
    Name = "${local.name_prefix}-nat-eiplab3"
  }
}

# Explanation: NAT is helga’s smuggler tunnel—private subnets can reach out without being seen.
resource "aws_nat_gateway" "helga_natlab3" {
  allocation_id = aws_eip.helga_nat_eiplab3.id
  subnet_id     = aws_subnet.helga_public_subnets[0].id # NAT in a public subnet

  tags = {
    Name = "${local.name_prefix}-natlab3"
  }

  depends_on = [aws_internet_gateway.helga_igwlab3]
}
...
resource "aws_route_table" "helga_public_rtlab3" {
  vpc_id = aws_vpc.helga_vpclab3.id

  tags = {
    Name = "${local.name_prefix}-public-rtlab3"
  }
}

# Explanation: This route is the Kessel Run—0.0.0.0/0 goes out the IGW.
resource "aws_route" "helga_public_default_route" {
  route_table_id         = aws_route_table.helga_public_rtlab3.id
  destination_cidr_block = "0.0.0.0/0"
  gateway_id             = aws_internet_gateway.helga_igwlab3.id
}
...
resource "aws_route_table" "helga_private_rtlab3" {
  vpc_id = aws_vpc.helga_vpclab3.id

  tags = {
    Name = "${local.name_prefix}-private-rtlab3"
  }
}
...
resource "aws_route" "helga_private_default_route" {
  ...
  destination_cidr_block = "0.0.0.0/0"
  nat_gateway_id         = aws_nat_gateway.helga_natlab3.id
}
...
resource "aws_security_group" "helga_ec2_sglab3" {
  name        = "${local.name_prefix}-ec2-sglab3"
...
resource "aws_security_group" "helga_rds_sglab3" {
  name        = "${local.name_prefix}-rds-sglab3"
  description = "RDS security group"
  vpc_id      = aws_vpc.helga_vpclab3.id

  # TODO: student adds inbound MySQL 3306 from aws_security_group.helga_ec2_sglab3.id
  ingress {
...
# adding in ingress rules for shinjuku_tgw to access RDS from Sao Paulo VPC via TGW
  ingress {
...
resource "aws_db_subnet_group" "helga_rds_subnet_grouplab3" {
  name       = "${local.name_prefix}-rds-subnet-grouplab3"
...
resource "aws_db_instance" "helga_rdslab3" {
  identifier             = "${local.name_prefix}-rdslab3"
  ...
  db_subnet_group_name   = aws_db_subnet_group.helga_rds_subnet_grouplab3.name
  vpc_security_group_ids = [aws_security_group.helga_rds_sglab3.id]
  ...
  tags = {
    Name = "${local.name_prefix}-rdslab3"
  }
}
...
resource "aws_iam_role" "helga_ec2_rolelab3" {
  name = "${local.name_prefix}-ec2-rolelab3"
...
resource "aws_iam_role_policy_attachment" "helga_ec2_ssm_attach" {
  role       = aws_iam_role.helga_ec2_rolelab3.name
...
resource "aws_iam_instance_profile" "helga_instance_profilelab3" {
  name = "${local.name_prefix}-instance-profilelab3"
  role = aws_iam_role.helga_ec2_rolelab3.name
}
...
resource "aws_ssm_parameter" "helga_db_port_param" {
  name  = "/lab/db/port"
  type  = "String"
  value = tostring(aws_db_instance.helga_rdslab3.port)
  overwrite = true
...
# Explanation: Secrets Manager is helga’s locked holster—credentials go here, not in code.
 resource "aws_secretsmanager_secret" "helga_db_secretlab3" {
  name = "${local.name_prefix}/rds/mysqlv2"
  #added two lines below to prevent deletion issues on terraform destroy for lab 1c Bonus B
...
data "aws_secretsmanager_secret" "helga_db_secretlab3" {
  name = "helga/rds/mysqlv2"  # or whatever the name is
}

data "aws_secretsmanager_secret_version" "helga_db_secret_versionlab3" {
  secret_id = data.aws_secretsmanager_secret.helga_db_secretlab3.id
} */

 
# Explanation: Secret payload—students should align this structure with their app (and support rotation later).
resource "aws_secretsmanager_secret_version" "helga_db_secret_versionlab3" {
  secret_id = aws_secretsmanager_secret.helga_db_secretlab3.id
  #secret_id = data.aws_secretsmanager_secret.helga_db_secretlab3.id

  secret_string = jsonencode({
    username = "admin"
    password = "i8$2>Iu]dQOh60lzzRG#iHJS1mg7"
    host     = "helga-rdslab3.chkce02amfxr.ap-northeast-1.rds.amazonaws.com"
    port     = 3306
    dbname   = "t_labdb"
  })
}
...
resource "aws_cloudwatch_log_group" "helga_log_grouplab3" {
  name              = "/aws/ec2/${local.name_prefix}-rds-app"
  retention_in_days = 7

  tags = {
    Name = "${local.name_prefix}-log-grouplab3"
  }
}
...
resource "aws_cloudwatch_metric_alarm" "helga_db_alarmlab3" {
  ...
  alarm_actions       = [aws_sns_topic.helga_sns_topiclab3.arn]
...
resource "aws_sns_topic" "helga_sns_topiclab3" {
...
resource "aws_sns_topic_subscription" "helga_sns_sublab3" {
  topic_arn = aws_sns_topic.helga_sns_topiclab3.arn
  protocol  = "email"
