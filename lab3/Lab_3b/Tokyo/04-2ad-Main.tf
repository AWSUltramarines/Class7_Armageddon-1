...
# - WAF logs must go to ap-northeast-1 for CLOUDFRONT scope
# - CloudWatch in ap-northeast-1 keeps everything in one region
# - Easier to query with Logs Insights

# CloudWatch Log Group (MUST be in ap-northeast-1, MUST start with "aws-waf-logs-")
...
/* resource "aws_wafv2_web_acl_logging_configuration" "helga_cf_waf_logging" {
  ...
  resource_arn            = aws_wafv2_web_acl.helga_cf_waflab3.arn
} */