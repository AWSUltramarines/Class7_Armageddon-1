...
output "saopaulo_peering_accepter_id" {
  ...
  #value       = var.enable_tgw && data.aws_ec2_transit_gateway.tokyo_tgw.id != "" ? aws_ec2_transit_gateway_peering_attachment_accepter.accept_tokyo_peer01[0].id : null
}
...
output "saopaulo_tgw_route_to_tokyo" {
  ...
  #value       = var.enable_tgw && data.aws_ec2_transit_gateway.tokyo_tgw.id != "" ? aws_ec2_transit_gateway_route.liberdade_route_to_tokyo[0].destination_cidr_block : null
}

output "liberdade_tgw_id" {
  value = var.enable_tgw ? aws_ec2_transit_gateway.liberdade_tgwlab3[0].id : ""
}