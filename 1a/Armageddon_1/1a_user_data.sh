#!/bin/bash
sudo FLASK_DEBUG=1 python3 app.py
dnf update -y
...
def get_conn():
    ...
    return pymysql.connect(host=host, user=user, password=password, port=port, database=db, autocommit=True, connect_timeout=5)
