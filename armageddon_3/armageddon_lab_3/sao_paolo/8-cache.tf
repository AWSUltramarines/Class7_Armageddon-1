...
resource "aws_cloudfront_cache_policy" "cache_static01" {
  ...
  min_ttl     = 1            # Force cache for at least 1s — prevents revalidation on every request
...
resource "aws_cloudfront_origin_request_policy" "orp_static01" {
  ...
  # Host is required so CloudFront can verify the ALB's TLS certificate
  # (cert covers jason-cramer.com, not the ALB DNS name).
  # This is forwarded-only — it does NOT enter the cache key.
  headers_config {
    header_behavior = "whitelist"
    headers { items = ["Host"] }
  }
...
# I moved these blocks inside the CloudFront distribution resource
