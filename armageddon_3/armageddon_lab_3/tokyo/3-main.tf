...
# SG referencing doesn't work across VPCs, so we use Liberdade's VPC CIDR explicitly.
resource "aws_security_group_rule" "shinjuku_rds_ingress_from_liberdade01" {
  ...
  to_port           = 3306
  protocol          = "tcp"
  cidr_blocks       = [var.liberdade_vpc_cidr]
}
...
resource "aws_iam_instance_profile" "ec2_profile01" {
  name = "${local.name_prefix}-shinjuku-ec2-profile"
  role = aws_iam_role.ec2_role01.name
}
...
resource "aws_iam_role" "ec2_role01" {
  name               = "${var.project_name}-shinjuku-ec2-role01"
