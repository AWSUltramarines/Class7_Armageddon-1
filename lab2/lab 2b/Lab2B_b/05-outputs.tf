...
output "helga_vpc_id" {
  value = aws_vpc.helga_vpclab2a.id
}
...
output "helga_ec2_instance_id" {
  value = aws_instance.helga_ec2lab2a.id
}
...
output "helga_sns_topic_arn" {
  value = aws_sns_topic.helga_sns_topiclab2a.arn
}

/* output "helga_log_group_name" {
  value = aws_cloudwatch_log_group.helga_log_grouplab2a.name
}
 */
#Bonus B Outputs for ALB and Target Group
output "helga_alb_dns_name" {
  value = aws_lb.helga_alblab2a.dns_name
}
...
output "helga_alb_logs_bucket_name" {
  value = var.enable_alb_access_logs ? aws_s3_bucket.helga_alb_logs_bucketlab2a[0].bucket : null
}
...
output "helga_waf_cw_log_group_name" {
  value = var.waf_log_destination == "cloudwatch" ? aws_cloudwatch_log_group.helga_waf_log_grouplab2a[0].name : null
}

output "helga_waf_logs_s3_bucket" {
  value = var.waf_log_destination == "s3" ? aws_s3_bucket.helga_waf_logs_bucketlab2a[0].bucket : null
}

output "helga_waf_firehose_name" {
  value = var.waf_log_destination == "firehose" ? aws_kinesis_firehose_delivery_stream.helga_waf_firehoselab2a[0].name : null
}
...
output "helga_waf_web_acl_arn" {
  ...
  value       = var.enable_waf ? aws_wafv2_web_acl.helga_waflab2a[0].arn : null
} */