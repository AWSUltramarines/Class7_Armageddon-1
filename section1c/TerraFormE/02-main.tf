...
resource "aws_lb" "chewbacca_alb01" {
  ...
  # Explanation: Access logs are the flight recorder—capture every request for forensics and compliance.
  # Real-world example: During a DDoS attack, logs reveal attacker IPs and patterns for WAF rule tuning.
  dynamic "access_logs" {
  ...
  # Explanation: Don't enable logging until S3 bucket policy grants ALB write permission
  depends_on = [
...
resource "aws_acm_certificate_validation" "chewbacca_acm_validation01" {
    ...
    validation_record_fqdns = [for record in aws_route53_record.chewbacca_cert_validation : record.fqdn]
}
...
# Add SQLi protection (priority 2)
rule {
...
# Add Bad Inputs protection (priority 3)
rule {
...
resource "aws_route53_zone" "chewbacca_zone" {
    name = var.domain_name  # weekinevents.com
...
# App DNS Record (ALIAS to ALB)
resource "aws_route53_record" "chewbacca_app_dns" {
    zone_id = aws_route53_zone.chewbacca_zone.zone_id
    name    = local.chewbacca_fqdn  # app.weekinevents.com
    type    = "A"
