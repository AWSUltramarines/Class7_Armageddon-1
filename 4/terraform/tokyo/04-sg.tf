...
# EC2 ingress rule — only allows HTTP traffic from the ALB security group
...
# RDS Security Group — controls inbound access to the MySQL database
...
# ALB ingress rule — allows HTTPS from CloudFront IPs using the AWS managed prefix list
...
# RDS ingress rule — allows MySQL from São Paulo VPC CIDR via Transit Gateway
resource "aws_vpc_security_group_ingress_rule" "rds_mysql_from_saopaulo" {
  security_group_id = aws_security_group.rds.id
  description       = "MySQL from Sao Paulo VPC via Transit Gateway"
  ip_protocol       = "tcp"
  from_port         = var.db_port
  to_port           = var.db_port

  # CIDR-based rule since SG references don't work cross-region
  cidr_ipv4 = var.saopaulo_vpc_cidr

  tags = {
    Name = "${var.project_name}-${var.environment}-rds-from-saopaulo"
  }
}

resource "aws_vpc_security_group_ingress_rule" "rds_mysql_from_iowa" {
  security_group_id = aws_security_group.rds.id
  description       = "MySQL from Iowa GCP VPC via VPN"
  ip_protocol       = "tcp"
  from_port         = var.db_port
  to_port           = var.db_port

  # CIDR-based rule since SG references don't work cross-cloud
  cidr_ipv4 = var.iowa_vpc_cidr

  tags = {
    Name = "${var.project_name}-${var.environment}-rds-from-iowa"
  }
}