...
# Construct the fully qualified domain name for the app subdomain (e.g. app.daequanbritt.com)
locals {
...
variable "saopaulo_vpc_cidr" {
  description = "CIDR block of São Paulo VPC for TGW routing"
  type        = string
  default     = "10.15.0.0/16" # Different from Tokyo's 10.14.0.0/16
}

# Add to 99-variables.tf (for Stage 3)
variable "saopaulo_tgw_id" {
  description = "São Paulo Transit Gateway ID for peering"
  type        = string
  default     = "" # Fill in after Stage 2 deployment
}

# Iowa (GCP) VPN variables — Stage 5
variable "iowa_vpc_cidr" {
  description = "GCP Iowa Nihonmachi VPC CIDR for TGW routing"
  type        = string
  default     = "10.16.0.0/16"
}

variable "gcp_vpn_external_ip_1" {
  description = "GCP HA VPN interface 0 external IP (from: terraform output vpn_gateway_interface_0_ip)"
  type        = string
  default     = "" # Fill in after Stage 4 GCP apply
}

variable "gcp_vpn_external_ip_2" {
  description = "GCP HA VPN interface 1 external IP (from: terraform output vpn_gateway_interface_1_ip)"
  type        = string
  default     = "" # Fill in after Stage 4 GCP apply
}

variable "vpn_psk_1" {
  description = "PSK for Iowa VPN tunnel 1 — generate with: openssl rand -hex 20"
  type        = string
  sensitive   = true
  default     = "" # Set via TF_VAR_vpn_psk_1 env var or a gitignored .tfvars file
}

variable "vpn_psk_2" {
  description = "PSK for Iowa VPN tunnel 2 — generate with: openssl rand -hex 20"
  type        = string
  sensitive   = true
  default     = "" # Set via TF_VAR_vpn_psk_2 env var or a gitignored .tfvars file
}