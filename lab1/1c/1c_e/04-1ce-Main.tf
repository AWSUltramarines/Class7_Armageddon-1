...
resource "aws_cloudwatch_log_group" "helga_waf_log_group01" {
  ...
  # NOTE: AWS requires WAF log destination names start with aws-waf-logs- (do not rename this).
...
resource "aws_wafv2_web_acl_logging_configuration" "helga_waf_logging01" {
...
# Explanation: S3 WAF logs are the long-term archive—receipts that survive dashboards.
...
# Explanation: Firehose needs a role—don't let random droids write into storage.
